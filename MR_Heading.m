% clc; clear; close
function [Partial_coef, Yhat, pval] = MR_Heading(Y,X,selvar)
% y = b1x1 + b2x2 + b3x3 + b4x4 + b5
% let's take data stored in matlab exemples
% load hald;
% Y=hald(:,5) ;
% X=hald(:,1:4);
X(:,[1,selvar]) = X(:,[selvar,1]);
% get a solution,
X = [X ones(length(X),1)];
...
t = sign(Partial_coef) .* Inf;
k = (abs(Partial_coef) < 1);
t(k) = Partial_coef(k) ./ sqrt(1-Partial_coef(k).^2);
t = sqrt(df).*t;
% switch tail
%     case 'b' % 'both or 'ne'
